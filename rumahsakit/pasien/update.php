...
if (isset($_POST['update'])) {
    ...
    $sql_update = "UPDATE t_pasien SET
                    f_norekmed = ?,
                    f_nama = ?,
                    f_tgllahir = ?,
                    f_jnskelamin = ?,
    ...
    try {
        ...
        $result = false;
        error_log("Update Pasien Error: " . $e->getMessage());
    }

    if ($result !== false) {
        $_SESSION['flash'] = 'success';
        $_SESSION['flash_message'] = 'Data pasien <strong>' . htmlspecialchars($nama) . '</strong> berhasil diupdate!';
        header("Location: select.php?id=$id_pasien");
        exit;
    } else {
        $_SESSION['flash'] = 'error';
        $_SESSION['flash_message'] = 'Gagal mengupdate data pasien. (Pastikan No. Rekam Medis belum terdaftar).';
        header("Location: update.php?id=$id_pasien");
        exit;
    }
}
...
<head>
    ...
    <title>Edit Pasien - <?= htmlspecialchars($pasien['f_nama']) ?> - RS InsanMedika</title>
            ...
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">
                        <i class="fas fa-user-edit"></i> Edit Data Pasien: <?= htmlspecialchars($pasien['f_nama']) ?>
                    </h1>
                    <a href="detail.php?id=<?= $id_pasien ?>" class="btn btn-secondary btn-sm shadow-sm">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-white">
                            <i class="fas fa-edit"></i> Form Edit Pasien
                        </h6>
                    </div>
                    
                    <div class="card-body">
                        <form method="POST" action="update.php?id=<?= $id_pasien ?>" id="formPasien">
                            
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="norekmed">
                                            <i class="fas fa-id-card text-primary"></i> No. Rekam Medis <span class="text-danger">*</span>
                                        </label>
                                        <input type="text" 
                                               name="norekmed" 
                                               id="norekmed" 
                                               class="form-control" 
                                               value="<?= htmlspecialchars($pasien['f_norekmed']) ?>" 
                                               required>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" 
                                               ...
                                               id="nama" 
                                               class="form-control" 
                                               value="<?= htmlspecialchars($pasien['f_nama']) ?>" 
                                               required>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="jnskelamin" 
                                                ...
                                                class="form-control" 
                                                required>
                                            <option value="Laki-laki" <?= $pasien['f_jnskelamin'] == 'Laki-laki' ? 'selected' : '' ?>>Laki-laki</option>
                                            <option value="Perempuan" <?= $pasien['f_jnskelamin'] == 'Perempuan' ? 'selected' : '' ?>>Perempuan</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="notlp">
                                            <i class="fas fa-phone text-primary"></i> No. Telepon <span class="text-danger">*</span>
                                        </label>
                                        <input type="text" 
                                               name="notlp" 
                                               id="notlp" 
                                               class="form-control" 
                                               value="<?= htmlspecialchars($pasien['f_notlp']) ?>" 
                                               required
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="alamat">
                                            <i class="fas fa-map-marker-alt text-primary"></i> Alamat <span class="text-danger">*</span>
                                        </label>
                                        <textarea name="alamat" 
                                                  ...
                                                  rows="3" 
                                                  required
                                                  placeholder="Masukkan alamat lengkap pasien"><?= htmlspecialchars($pasien['f_alamat']) ?></textarea>
                                    </div>
                                </div>
                            </div>
